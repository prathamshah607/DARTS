# -*- coding: utf-8 -*-
"""graph_builder_edges_plot.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1VR1XPDuCH0AfUiQ_V4Y3sDw-nYD4oShL
"""
...
"""**2. Code Snapping the Nodes to the Graph**"""
...
"""**-- To get the edges (lengths) from the graph --**"""
...
"""--functions for the application and lifting of closures--"""
...
"""--simulating the closure and lift using plots at each timestep--"""
...
import time
closed_edges = set()
node_ids = [val['graph_node'] for val in graph.values()]

for current_timestep in range(1, 30):
    with open("/content/closures.json") as f:
        data = json.load(f)

    events_this_step = [e for e in data["events"] if e["timestep"] == current_timestep]

    for event in events_this_step:
        u, v, action = event["u"], event["v"], event["action"]

        if action == "close":
            apply_closure(G, u, v)
            closed_edges.add((u, v))
            desc = f"Closed ({u}, {v})"
            print(f"[t={current_timestep}] {desc}")

        elif action == "lifted":
            lift_closure(G, u, v)
            closed_edges.discard((u, v))
            desc = f"Lifted ({u}, {v})"
            print(f"[t={current_timestep}] {desc}")
